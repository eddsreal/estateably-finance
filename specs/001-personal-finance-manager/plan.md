...
from `APP_TIMEZONE` (FR-029); env vars closed to the FR-035 six (incl. `CORS_ORIGINS`,
amended 2026-09-20); structured errors with
...
_Gate evaluated before Phase 0 and re-checked after Phase 1 design: **PASS** (both), with two
justified entries in Complexity Tracking, no violations._
...
| IV. Contracts before code  | `contracts/openapi.yaml` + `contracts/schema.prisma` frozen with this plan. Web types generated by openapi-typescript; hand-written contract types banned (lint + review). Drift guards: `contract:check` (routes vs yaml, CI) and ajv validation of every e2e response. Changes re-enter through the plan.                                                                                                                                                                   |
...
them). Everything else is dev-only or already mandated.
...
packages/contract/
├── openapi.yaml                 # the frozen contract (source of truth for codegen and /docs)
└── src/types.ts                 # generated by openapi-typescript — never edited by hand
...
`/docs` static pipeline, CI green end to end.
...
- **Forms** (React Hook Form, no resolver, no zod): value types derive from generated
  ...
  TanStack Query mutations; success invalidates balance queries from `query-keys.ts` (FR-028).
...
catalogue, security headers + CORS, ajv against the yaml per endpoint/status. UI e2e
