...
│   ├── app.module.ts
│   ├── common/                  # NO business logic; one folder per unit: <name>/<name>.ts + its .spec.ts
│   │   ├── prisma.service/prisma.service.ts
│   │   ├── correlation-id.middleware/correlation-id.middleware.ts
│   │   ├── logging.interceptor/logging.interceptor.ts   # json ConsoleLogger, 1 line/request, no amounts/descriptions
│   │   ├── domain-errors/domain-errors.ts               # + colocated .spec.ts
│   │   ├── http-exception.filter/http-exception.filter.ts   # + colocated .spec.ts — the ONLY error→HTTP mapping
│   │   └── money.decorator/money.decorator.ts           # + colocated .spec.ts — pattern + ±10^15 guard after BigInt
...
| **(triggered 2026-09-22)** Prisma 7 config model: datasource URL out of the schema, driver adapter in                                                                          | Foundational verification: Prisma 7.10 hard-rejects `url = env(...)` inside schema files and `PrismaClient` refuses to construct without a driver adapter. The frozen `contracts/schema.prisma` was amended (datasource `url` line removed — no model, column or type changed); `apps/api/prisma.config.ts` now carries the URL (from `DATABASE_URL`) and the migrations/seed wiring; `@prisma/adapter-pg@7.10.0` added as the api's one new runtime dependency (Prisma's own Postgres adapter, version-locked to the client)  | Staying byte-compatible was impossible: the file does not parse under the pinned Prisma 7.10 (R-001), and downgrading to Prisma 6 contradicts R-001's explicit pin. Amendment is mechanical and forced; pending ratification at the Foundational checkpoint                                                                                                                                                          |
| **(triggered 2026-09-22)** TS6 scoped to openapi-typescript                                                                                                                    | Setup verification confirmed openapi-typescript 7.13 needs the TS6 JS compiler API (`ts.factory`), absent from TS7. `packages/contract` pins `typescript: 6.0.3` as its own devDependency, so only the codegen tool resolves TS6                                                                                                                                                                                                                                                                                               | Downgrading the project to TS6 rejected: directive fixes TS7. The `openapi-typescript>typescript` pnpm override cannot redirect peer resolution, so the pin lives in the one package that runs the tool; that package contains only the generated `types.ts`, which TS7 typechecks where it is consumed (apps/web). Ceiling (recorded here — CLAUDE.md bans code comments): drop the pin when the tool supports TS7. |
| **(recorded 2026-09-22, US1)** Web API base URL as the constant `http://localhost:3000` in `apps/web/src/shared/lib/api.ts`                                                    | The stack fixes both origins (research R-004: SPA on 5173/8080, API on 3000, no proxy), so a constant is the whole requirement; T042 asked for the ceiling to be recorded, and CLAUDE.md routes ceilings here instead of code comments. Ceiling: promote to build-time configuration (`import.meta.env`) if the API origin ever differs per environment                                                                                                                                                                        | An env-driven value today would be dead configuration: no environment exists where it varies                                                                                                                                                                                                                                                                                                                         |
| **(recorded 2026-09-22, US1)** jsx-a11y exceptions scoped in `.oxlintrc.json`                                                                                                  | `design/design-system.md` mandates custom pickers (ARIA combobox/listbox pattern on button+ul, per the W3C APG select-only combobox) and the plan adopts amount autofocus on create; `prefer-tag-over-role`, `no-noninteractive-element-to-interactive-role` (Picker only) and `no-autofocus` (web src) are turned off exactly where the design demands the pattern                                                                                                                                                            | Native `<select>` rejected by the design system ("Pickers — ours, never the browser's"); dropping autofocus rejected by the adopted prototype interaction                                                                                                                                                                                                                                                            |
| **(recorded 2026-09-23, US6)** LLM model as the constant `LLM_MODEL = 'claude-haiku-4-5-20251001'` in `apps/api/src/modules/ai/services/ai.service.ts`                         | R-012 fixes one economical model; T077 asked for a ceiling comment, and CLAUDE.md routes ceilings here instead. Ceiling: promote to configuration if model choice ever needs to vary per environment (that is a new env var, so it re-enters FR-035 first)                                                                                                                                                                                                                                                                     | An env var today would be dead configuration and would break FR-035's closed list                                                                                                                                                                                                                                                                                                                                    |
| _(retired by feature 002: `GET /ai/status`, research R-004)_ **(recorded 2026-09-23, US6)** Narrative button learns "not configured" from the first `AI_NOT_CONFIGURED` answer | The frozen contract exposes no AI-availability flag, and the key is server-only. The button is enabled once a report exists; the first `422 AI_NOT_CONFIGURED` disables it for the page's lifetime with the design's tooltip and caption, no notice. Ceiling: disabled up front (as the prototype shows) needs a contract amendment exposing availability                                                                                                                                                                      | Probing the narrative endpoint on load would spend a real provider call whenever a key is set; a build-time web flag would duplicate server configuration                                                                                                                                                                                                                                                            |
| _(not triggered)_ SWC build for the api                                                                                                                                        | Setup verification 2026-09-22: TS7 `tsc` emits `design:paramtypes` metadata for the Nest + class-validator sample — no fallback needed. Foundational addendum 2026-09-24: the Nest CLI's `nest build`/`nest start` refuse to run under TS7 (they need the removed compiler API), so the api's `build`/`dev`/`contract:check` scripts invoke `tsc -p tsconfig.build.json` + `node dist/main.js` directly — exactly the "tsc for typecheck and build" R-002 already decided; `dev` runs without watch until the CLI supports TS7 | Hand-rolled decorator shims rejected; SWC ships inside `@nestjs/cli`, `tsc` (TS7) stays the typechecker.                                                                                                                                                                                                                                                                                                             |
