...
- **FR-035**: The configuration this feature introduces MUST be read from these environment variables, each listed in `.env.example` with its default: `APP_TIMEZONE`, the timezone that defines today for FR-029 (default `UTC`); `LOG_LEVEL`, for FR-033 (default `info`); `LLM_API_KEY`, the provider key of FR-015 (default empty, meaning not configured); `LLM_TIMEOUT_MS`, the provider timeout of FR-015 (default `10000`); and `CORS_ORIGINS`, the comma-separated allowlist of browser origins the API accepts cross-origin requests from (default `http://localhost:5173,http://localhost:8080`) _(amended 2026-09-20 during planning)_.
