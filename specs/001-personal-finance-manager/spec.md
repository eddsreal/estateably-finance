...
**Status**: Approved (`review-spec` 2026-09-19; `review-plan` with frozen contract 2026-09-21); tasks generated and analyzed 2026-09-22 — ready for `/speckit-implement` Phase 1
...
### Session 2026-09-22 (analysis)

- Q: HSTS depends on `NODE_ENV` (plan decision R-006), which FR-035 did not list — is the environment-variable set still closed? → A: FR-035 is amended to add `NODE_ENV` as its seventh variable (default `development`; Docker Compose sets `production`). The API emits HSTS only when it equals `production`, and `.env.example` lists it like the rest.
- Q: May a category's type (_expense_/_income_) be edited after creation? → A: Only while unused. While no transaction, soft-deleted included, references the category the type may be changed; once used it is immutable and a change is rejected with the structured error. FR-003 and `CategoryUpdateRequest` in the contract are amended accordingly.
- Q: May a project budget be zero? → A: No. When present it is a positive integer number of cents; zero or negative is rejected with the structured error. "No budget" is expressed by omitting the field. FR-019 is amended accordingly.
...
- **FR-003**: The system MUST ship with a seeded catalogue of 12 categories, each typed _expense_ or _income_, and MUST let the user create, rename, archive and unarchive categories. A category's type MAY be changed only while no transaction, soft-deleted ones included, references the category; once used, the type is immutable and a change MUST be rejected with the FR-023 structured error _(amended 2026-09-22 during analysis)_. The catalogue is exactly: _Groceries_, _Dining_, _Rent_, _Utilities_, _Transport_, _Health_, _Entertainment_, _Shopping_ and _Travel_ (expense), and _Salary_, _Freelance_ and _Interest_ (income).
...
- **FR-019**: The system MUST let the user create, edit, close, reopen and (when unused) delete projects with a name, an optional budget and a status. Closing and reopening are reversible at any time. The budget, when present, MUST be a positive integer number of cents; zero or negative MUST be rejected with the FR-023 structured error — "no budget" is expressed by omitting the field, never by zero _(amended 2026-09-22 during analysis)_.
...
- **FR-035**: The configuration this feature introduces MUST be read from these environment variables, each listed in `.env.example` with its default: `APP_TIMEZONE`, the timezone that defines today for FR-029 (default `UTC`); `LOG_LEVEL`, for FR-033 (default `info`); `LLM_API_KEY`, the provider key of FR-015 (default empty, meaning not configured); `LLM_TIMEOUT_MS`, the provider timeout of FR-015 (default `10000`); `LLM_BASE_URL`, the base URL of the FR-015 provider (default `https://api.anthropic.com`; automated tests point it at a local stub) _(amended 2026-09-21 during plan review)_; `CORS_ORIGINS`, the comma-separated allowlist of browser origins the API accepts cross-origin requests from (default `http://localhost:5173,http://localhost:8080`) _(amended 2026-09-20 during planning)_; and `NODE_ENV`, the standard Node runtime mode, which this feature reads only to decide whether the API emits HSTS (emitted when it equals `production`; default `development`; the Docker Compose stack sets `production`) _(amended 2026-09-22 during analysis)_.
