...
`@tailwindcss/vite` 4.3.3** (R-013), which are build-time only. **New runtime dependency:
`recharts` 3.10.1** for the chart, sparklines and donut (R-007, user decision 2026-09-24).
Toasts, palette, swipe and keypad are hand-written (R-008 to R-010). The
...
Stack constraints: one new runtime dependency, `recharts` 3.10.1, exact-pinned. Reason: the user
chose a charting library over hand-drawn SVG on 2026-09-24, so drawing, scaling and the range
morph come from a maintained library instead of project code (R-007). Reason stated for the two
new devDependencies:
...
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `money.ts#toPlotNumber`: cents → `number`, for chart geometry only | Recharts plots `number` values. Principle II forbids `number` arithmetic on cents "where precision could be lost". Here the value is only turned into pixels: it is never displayed, summed or compared, and every label reads the exact `bigint` string. Ceiling: none that matters for display. Precision loss above 2^53 cents (about $90 trillion) moves a point by less than one pixel. Enforced by keeping it the only conversion in `money.ts` and asserting in `LineChart`/`Donut` tests that every rendered figure equals the source string. This ceiling and the ones below are recorded here and not in code comments: as constitution v1.1.0 and CLAUDE.md require. | Plotting in `bigint`: Recharts cannot take bigint values, so every value would still become a `number` at the library boundary. Converting in one named place is the auditable version. |
...
| Charts plot every day of the series (R-007)                        | Recharts gets the whole series, so hover, drag and keys select the exact day with no sampling. Ceiling: "All" over 10 years draws about 3 650 points in the chart and in each sparkline. Upgrade path: pass Recharts a min/max-preserving downsample while selection keeps reading the full series.                                                                                                                                                                                                                                                                                                                                                                             | Downsampling now: a second index mapping between drawn points and days, with no measured need.                                                                                          |
